...
const EventDetail = () => {
    ...
    // Colour helpers — used for B+C tab theming
    const accentColor = event.color || '#6366f1';
    // 0D = 5% opacity in hex — subtle tint, text always readable
    const tintBg = accentColor + '0D';

    return (
        <div>
            {/* Subtle colour tint applied to the whole page content area */}
            <div
                className="min-h-screen rounded-xl transition-colors duration-300"
                        ...
                        {tabs.map((tab) => {
                            ...
                            return (
                                <button
                                    ...
                                    className={`flex items-center gap-2 py-4 px-2 border-b-2 font-medium text-sm transition-colors whitespace-nowrap ${isActive
                                        ? 'border-transparent'  // border set via inline style below
